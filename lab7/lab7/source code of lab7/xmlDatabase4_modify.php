...
if(isset($_POST['ISBN']) && isset($_POST['Action']))
{
	if(isset($_POST['oldValue']))
		$oldValue = addslashes($_POST['oldValue']);
	else
		$oldValue = "";
	$ISBN = $_POST['ISBN'];
	...
	if(($_POST['addnewValue'])!="")
	{
		$addnewValue = addslashes($_POST['addnewValue']);
		if($_POST['Action'] == "Add")
		{
...
header("Location: xmlDatabase4_modify.php");
}
...
// to actually complete the query we need to POST rid of them.
...
while ($row = mysql_fetch_row($res)) {
	...
	print "<td>$row[1]</td>"; //display the title returned
	{
		...
		while($row1 = mysql_fetch_row($res1))
		{
			...
			while($loop < $row[3])
			{
				@print "<tr><td><table><tr><td><form action=\"$PHP_SELF\" method=\"POST\">
	...
	print @"<td><table><tr><td><form action=\"$PHP_SELF\" method=\"POST\">
					<input type=\"hidden\" name=\"ISBN\" value=\"$row[0]\"/>
